...
string labeling is called FASTA format. In this format, the string is introduced by a line that begins with '>',
...
file = FileHandler(path=sys.argv[1])
sequences = file.load_fasta()

max_g_c_content = 0
max_g_c_sequence_id = None
for identifier in sequences.keys():
    dna = DNA(sequences[identifier])
    g_c_content = dna.count_gc_content()
    if g_c_content > max_g_c_content:
        max_g_c_content = g_c_content
        max_g_c_sequence_id = identifier
print(max_g_c_sequence_id)
print(max_g_c_content * 100)
